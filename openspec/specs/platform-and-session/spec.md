# platform-and-session Specification

## Purpose

Captures the existing Catalyst Flutter web app platform: startup, API access, session persistence, route protection, and shared error handling.

## Requirements

### [PRE] Requirement: Application platform

The system SHALL start in a consistent web/mobile-style shell so users can use Catalyst in portrait without debug chrome.

#### Scenario: Orientation lock
- **WHEN** the app starts
- **THEN** the system SHALL lock orientation to portrait up and portrait down

#### Scenario: Local cache before first route
- **WHEN** the app starts
- **THEN** the system SHALL initialize local cache before showing the first route

#### Scenario: Splash is first route
- **WHEN** the app starts
- **THEN** the system SHALL open the splash route `/`

#### Scenario: Debug banner hidden
- **WHEN** the app is running
- **THEN** the system SHALL hide the Flutter debug banner

#### Scenario: Outside tap unfocuses
- **WHEN** the user taps outside a focused field
- **THEN** the system SHALL unfocus the current input

#### Scenario: Web hash URLs
- **WHEN** the app is served on web
- **THEN** named routes SHALL be hash URLs

### [PRE] Requirement: API base URL

The system SHALL set the API host at compile time so local and production builds can target different backends.

#### Scenario: Dart-define override
- **WHEN** `API_BASE_URL` is passed via `--dart-define`
- **THEN** the system SHALL use that value as the API base URL
...
The system SHALL send the access token on API calls and fail in a consistent way when the server cannot be reached or the session is invalid.

#### Scenario: Default JSON content type
- **WHEN** a request requires default headers
- **THEN** the system SHALL send `Content-Type: application/json`

#### Scenario: Bearer token when present
- **WHEN** a request requires default headers AND an access token exists
- **THEN** the system SHALL send `Authorization: Bearer <token>`

#### Scenario: Login form encoding
- **WHEN** a login request is sent
- **THEN** the system SHALL send `application/x-www-form-urlencoded` without the Authorization header

#### Scenario: Optional headers omitted
- **WHEN** a request does not require default headers
- **THEN** the system SHALL omit the Authorization header

#### Scenario: JSON success
- **WHEN** a request succeeds with HTTP 200 or 201
- **THEN** the system SHALL decode JSON maps or lists from the body

#### Scenario: Empty success
- **WHEN** a request succeeds with HTTP 204
- **THEN** the system SHALL treat the response as an empty map

#### Scenario: Timeout snackbar
- **WHEN** a request times out (default 5 seconds, LLM 30 seconds)
- **THEN** the system SHALL show a top snackbar `Error` / `Could not reach the server. Please try again.` and return no payload

#### Scenario: Network error snackbar
- **WHEN** a socket or unexpected network error occurs
- **THEN** the system SHALL show the same unreachable-server snackbar

#### Scenario: Rate limit
- **WHEN** the server returns HTTP 429
- **THEN** the system SHALL show `Too many requests exception, please try again later!`

#### Scenario: Detail error snackbar
- **WHEN** the server returns HTTP 409 or another handled error with a `detail` field
- **THEN** the system SHALL show that detail text in an `Error` snackbar

#### Scenario: FastAPI validation list
- **WHEN** FastAPI returns `detail` as a list of objects
- **THEN** the system SHALL join each object's `msg` into the snackbar text

### [PRE] Requirement: Session storage

The system SHALL restore a returning user's access token on reload so they do not have to log in again.

#### Scenario: Persist token
- **WHEN** an access token is persisted
- **THEN** the system SHALL store it in GetStorage database `catalyst_local_cache` under key `access_token`

#### Scenario: Restore token
- **WHEN** the app starts
- **THEN** the system SHALL restore a non-empty trimmed `access_token` into memory

#### Scenario: Login persists session
- **WHEN** login succeeds
- **THEN** the system SHALL persist `access_token` or `token` from the response and set local cache `user_logged_in_status` to logged in

#### Scenario: Clear session
- **WHEN** the session is cleared
- **THEN** the system SHALL remove the access token, clear in-memory user state, reset onboarding complete to false, clear admin cookies, and set `user_logged_in_status` to logged out

#### Scenario: Empty token is unauthenticated
- **WHEN** `hasAccessToken` is evaluated
- **THEN** the system SHALL treat a missing, empty, or whitespace-only token as unauthenticated

### [PRE] Requirement: Role and admin session

The system SHALL distinguish admin JWTs from member JWTs so admins cannot use member screens and members cannot use the admin console.

#### Scenario: Admin role
- **WHEN** a JWT payload contains `role` equal to `admin`
- **THEN** the system SHALL treat the session as an admin session

#### Scenario: Decode failure is non-admin
- **WHEN** JWT payload decoding fails
- **THEN** the system SHALL treat the session as non-admin

#### Scenario: Admin cookies on login
- **WHEN** an admin logs in
- **THEN** the system SHALL also store `admin_token`, `admin_name`, and optional `admin_expires_at` cookies for 7 days on web

#### Scenario: Admin cookies cleared
- **WHEN** the session is cleared
- **THEN** the system SHALL delete those admin cookies

### [PRE] Requirement: Route protection
...
- **THEN** the system SHALL leave them on that public screen and SHALL not bounce them to `/base` or `/admin-welcome`
...
- **THEN** the system SHALL clear the session and redirect to `/auth`, or `/admin` if the current route contains `admin`
...
### [PRE] Requirement: Named routes

The system SHALL expose stable URLs for each screen so email links and bookmarks work.
...
The system SHALL record unused modules so later specs do not treat them as live product.

#### Scenario: Live home UI
- **WHEN** documenting live member home
- **THEN** the system SHALL treat `StackedCardsScreen` inside `/base` as the home UI, not the unused `HomeNavigationBar` / `home_view` / Appinio `swipe_cards` feature
