...
Captures existing first-time profile onboarding: the init form and optional LLM-drafted description.

## Requirements

### [PRE] Requirement: Access

The system SHALL gate onboarding behind the member session so only that member can complete their profile.
...
### [PRE] Requirement: Init form fields

The system SHALL let a new member enter who they are so other users can see their card.
...
- **THEN** the options SHALL be: PhD Student; Postdoctoral researcher; Junior Professor; Professor; Student; Academic Staff; Group Leader; Other

#### Scenario: Affiliation search
- **WHEN** the user types 3 or more characters in Affiliation
- **THEN** after 400ms the system SHALL GET `/profile/affiliations?q=` and show suggestions with `label` and subtitle `country_name - name`

#### Scenario: Short affiliation query
- **WHEN** the query is shorter than 3 characters
- **THEN** affiliation suggestions SHALL be cleared
...
- **THEN** the system SHALL add it as a removable chip and clear the keyword field

#### Scenario: Continue flushes keyword field
- **WHEN** Continue is pressed
- **THEN** any non-empty keyword still in the text field SHALL be added first

### [PRE] Requirement: Init form submit
...
- **THEN** Continue SHALL show `Please enter a description or at least one keyword.`

#### Scenario: Save without keywords
- **WHEN** Continue is valid AND there are no keywords
- **THEN** the system SHALL PATCH `/profile/me` with filled non-empty fields and `onboarding_complete: true`, then navigate to `/base`

#### Scenario: Keywords trigger LLM
- **WHEN** Continue is valid AND keywords exist
- **THEN** the system SHALL PATCH `/profile/me` with filled non-empty fields without marking onboarding complete, then POST `/profile/llm` with `{ "keywords" }` using a 30-second timeout

#### Scenario: LLM drafts
- **WHEN** the LLM response includes `draft_1` and `draft_2`
- **THEN** the system SHALL navigate to `/llm-choice` with those drafts (also aliased as `string1` / `string2`)

#### Scenario: Profile save failure
- **WHEN** profile save fails
- **THEN** the system SHALL show `Could not save your profile. Please try again.`

#### Scenario: LLM fetch failure
- **WHEN** LLM fetch fails
- **THEN** the system SHALL show `Could not fetch suggestions. Please try again.` and remain on the init form

#### Scenario: Saving state
- **WHEN** Continue is in progress
- **THEN** the button SHALL show `Saving...` and ignore extra taps

### [PRE] Requirement: LLM description choice

The system SHALL let a new member pick or write a profile description from AI drafts so their card has usable copy.

#### Scenario: Choice title
- **WHEN** `/llm-choice` opens
- **THEN** the system SHALL title the screen `Choose Your Preference` and ask which description the user prefers

#### Scenario: Draft options
- **WHEN** drafts are present
- **THEN** the system SHALL list each non-empty draft plus a final option `Write your own description...`

#### Scenario: Continue disabled
- **WHEN** no option is selected
- **THEN** `Continue and Edit` SHALL be disabled

#### Scenario: Enter edit step
- **WHEN** the user continues from the list
- **THEN** the system SHALL enter an edit step with the chosen draft, or an empty field for the custom option

#### Scenario: Empty edit description
- **WHEN** the edit-step description is empty
- **THEN** Continue SHALL show `Please write a description before continuing.`

#### Scenario: Finalize description
- **WHEN** the edit-step description is non-empty
- **THEN** the system SHALL PATCH `/profile/me` with that `description` and `onboarding_complete: true`, then navigate to `/base`
...
- **THEN** the system SHALL return to `/initform` because `/llm-choice` was pushed, not replaced
